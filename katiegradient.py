...
def derivx(vel,xcoords):
	...
	for i in range(len(vel)):
		for x in range(len(vel)):
			if 0 < i < len(vel) - 1:
				velxdx[i,x] = ((-1/2) * velxdx[i - 1][x]) + ((1/2) * velxdx[i + 1][x])/distance
			elif i == 0:
				velxdx[i,x] = (((-3/2) * velxdx[i][x]) + (2 * velxdx[i + 1][x]) + ((-1/2) * velxdx[i+2][x]))/distance
			elif i == len(vel) - 1:
				velxdx[i,x] = ((-3/2) * velxdx[i][x]) + (2 * velxdx[i - 1][x]) + ((-1/2) * velxdx[i-2][x])/distance
...
def derivy(vel,xcoords):
	...
	for i in range(len(vel)):
		for x in range(len(vel)):
			if 0 < x < len(vel) - 1:
				velydy[i,x] = (((-1/2) * velydy[i][x - 1]) + ((1/2) * velydy[i][x + 1]))/distance
			elif x == 0:
				velydy[i,x] = (((-3/2)*velydy[i][x]) + (2*velydy[i][x+1])+((-1/2) * velydy[i][x + 2]))/distance
			elif x == len(vel) - 1:
				velydy[i,x] = (((-3/2)*velydy[i][x]) + (2*velydy[i][x - 1]) + ((-1/2) * velydy[i][x - 2]))/distance
